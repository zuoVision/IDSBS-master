    ...
    with open(file_name, 'r') as f:
        for row in csv.reader(f):
            ...
            for i in range(len(row)-1):
                feature_tmp.append(float(row[i]))
            feature_data.append(feature_tmp)
            label_tmp.append(float(row[-1]))
        print(np.array(feature_data))
        print(np.array(label_tmp))
...
if __name__ == '__main__':
    ...
    hidden_nodes = 20
    input_data_trainX = x_data
    input_data_trainY = y_data
    print(input_data_trainX[1])
